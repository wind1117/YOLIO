...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            evt = cv2.resize(evt, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = cv2.copyMakeBorder(
            img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        evt = cv2.copyMakeBorder(
            evt, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels['evt'] = self._format_img(labels.pop('evt'))
...
class RectImgTest:
    def __init__(self, imgsz=384):
        self.imgsz = imgsz

    def __call__(self, batch):
        h0, w0 = batch['img'].shape[:2]
        r = self.imgsz / max(h0, w0)
        if r != 1:
            w, h = (min(math.ceil(w0 * r), self.imgsz), min(math.ceil(h0 * r), self.imgsz))
            batch['img'] = cv2.resize(batch['img'], (w, h), interpolation=cv2.INTER_LINEAR)
            batch['evt'] = cv2.resize(batch['evt'], (w, h), interpolation=cv2.INTER_LINEAR)
        batch['resized_shape'] = batch['img'].shape[:2]
        batch['ratio_pad'] = (
            batch['resized_shape'][0] / batch['ori_shape'][0],
            batch['resized_shape'][1] / batch['ori_shape'][1],
        )
        return batch
...
class PreProcTest:
    ...
    def __call__(self, batch):
        ...
        batch['img'] = (batch['img'].float()) / 255  # assume no half needed
        batch['evt'] = batch['evt'].to(self.device, non_blocking=True)
        batch['evt'] = (batch['evt'].float()) / 255
...
class ManualBatch:
    def __call__(self, batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k in ['im_file', 'ev_file', 'ori_shape', 'resized_shape', 'ratio_pad']:
                value = [value]
            new_batch[k] = value
        return new_batch
