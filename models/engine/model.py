...
class Model(nn.Module):
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        # \/ def attempt load one weight
        self.model = torch.load(weights)['model'].to(self.device).float()
        ...
        for m in self.model.modules():
            if hasattr(m, "inplace"):
                m.inplace = True  # arg inplace = True
            elif isinstance(m, nn.Upsample) and not hasattr(m, "recompute_scale_factor"):
                m.recompute_scale_factor = None
        # /\ def attempt load one weight

        self.model.args = {
            'imgsz': 384, 'data': 'cfg/datasets/coco_pku.yaml',
            'task': self.task, 'single_cls': False
        }
    ...
    def infer(
        ...
        **kwargs,
        ...
        custom = {"rect": True}  # method defaults
        args = {**self.overrides, **custom, **kwargs, "mode": "test"}  # highest priority args on the right

        predictor = (predictor or self._smart_load("infer"))(args=args, _callbacks=self.callbacks)
...
def build_model_load_weights(weights, nc=80):
    model = DetectionModel(nc=nc)
    model.model.load_state_dict(torch.load(weights, map_location='cpu'), strict=False)
    return model
